...
export async function createEmployee(db, tenantId, payload) {
  const ref = await addDoc(collection(db, tenantCollectionPath(tenantId, COLLECTIONS.EMPLOYEES)), { ...createEmployeeModel({ ...payload, tenantId }), createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
  return ref.id;
}

export async function getEmployee(db, tenantId, employeeId) {
  return docToEntity(await getDoc(doc(db, tenantCollectionPath(tenantId, COLLECTIONS.EMPLOYEES), employeeId)));
}
...
export async function updateEmployee(db, tenantId, employeeId, patch) {
  await updateDoc(doc(db, tenantCollectionPath(tenantId, COLLECTIONS.EMPLOYEES), employeeId), { ...patch, updatedAt: serverTimestamp() });
}
