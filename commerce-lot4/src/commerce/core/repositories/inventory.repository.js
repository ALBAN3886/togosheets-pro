import { collection, doc, getDoc, getDocs, setDoc, updateDoc, query, where, onSnapshot, serverTimestamp, increment } from '../firebase/firestore.js';
...
export async function getInventoryByProduct(db, tenantId, storeId, productId) {
  return docToEntity(await getDoc(inventoryDocRef(db, tenantId, storeId, productId)));
}
...
export async function setInventoryQuantity(db, tenantId, storeId, productId, payload) {
  await setDoc(inventoryDocRef(db, tenantId, storeId, productId), { ...createInventoryModel({ ...payload, tenantId, storeId, productId }), updatedAt: serverTimestamp() }, { merge: true });
}
