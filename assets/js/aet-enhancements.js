...
(function(){
  ...
  // Notifications push désactivées : l'envoi nécessite une Cloud Function,
  // donc le plan Firebase Blaze. Les alertes de commande restent assurées
  // par le badge, le toast et la carte permanente dans la discussion.
  window.aetxRequestPushPermission = async function() {
    notify('Notifications push indisponibles sur la formule actuelle', 'info');
  };

  window.aetxDisablePushNotifications = async function() {
    notify('Notifications push non activees', 'info');
  };
