...
export async function createCategory(db, tenantId, payload) {
  const model = createCategoryModel({ ...payload, tenantId });
  const ref = await addDoc(collection(db, tenantCollectionPath(tenantId, COLLECTIONS.CATEGORIES)), {
    ...model,
...
export async function getCategory(db, tenantId, categoryId) {
  const snap = await getDoc(doc(db, tenantCollectionPath(tenantId, COLLECTIONS.CATEGORIES), categoryId));
...
export async function updateCategory(db, tenantId, categoryId, patch) {
  await updateDoc(doc(db, tenantCollectionPath(tenantId, COLLECTIONS.CATEGORIES), categoryId), {
    ...patch,
    updatedAt: serverTimestamp(),
  });
}
