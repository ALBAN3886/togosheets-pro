...
import { ensureOwnerBootstrap, getCommerceContext, migrateLegacyCommerce, saveStore, buildDashboardStats, buildSalesReport } from '../../core/services/index.js';
...
import { saveCategory, saveProduct } from '../../core/services/catalog.service.js';
...
async function renderPage() {
  ...
  if (page === 'reports') {
    const [sales, orders, inventory, cashSessions] = await Promise.all([
    ...
    frame(renderReportsPage(buildSalesReport({ sales, orders, inventory, cashSessions })), 'Rapports');
    return;
  }
  ...
  const dashboardStats = {
    ...buildDashboardStats({ sales, orders, inventory, cashSessions }),
    ...buildSalesReport({ sales, orders, inventory, cashSessions })
  };
  frame(renderAdminDashboard(dashboardStats), 'Dashboard');
}
